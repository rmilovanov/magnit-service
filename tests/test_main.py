...
def test_post_calc(monkeypatch, case):
    ...
    class MockTask:
        id = "test_task_id"
    monkeypatch.setattr(main.make_computations, "delay", mock.MagicMock(return_value=MockTask))
    ...
    if response.status_code == 200:
        ...
        assert response.json()["detail"][0]["msg"] == "Supported operations are ['+', '-', '*', '/']"
...
def test_get_task_result(monkeypatch):
    ...
    monkeypatch.setattr(main, "AsyncResult", mock.MagicMock(return_value=MockTaskResult))

    response = client.get("/task/test_task_id1/")
    assert response.status_code == 200
    assert response.json() == {'task_id': 'test_task_id1', 'task_result': 124, 'task_status': 'SUCCESS'}